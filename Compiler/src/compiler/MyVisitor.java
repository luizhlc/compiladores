...
public class MyVisitor extends LHCBaseVisitor<String> {
	...
	public String visitProgram(ProgramContext ctx) {
		...
		if (!methods.containsKey("main")) {
			try {
				throw new Exception(
						"No main method... Are you sure you've picked up the right course?");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public String visitMethodDef(MethodDefContext ctx) {
			...
			if (ctx.returnExp.getText().equals("void")) {
				try {
					throw new Exception("line:"
							+ ctx.type_.getStart().getLine()
							+ "\n... you need to return something in a "
							+ ctx.type_ + " method.");
				} catch (Exception e) {
					e.printStackTrace();
				}
			...
			if (!ctx.type_.getText().equals(type_st.peek().getType())) {
				try {
					throw new Exception("line:"
							...
							+ type_st.peek().getType()
							+ " return??? ... You need to return a "
							+ ctx.type_.getText() + " in a "
							+ ctx.type_.getText() + " method.");
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
		...
		if (ctx.funcName.getText().equals("main")) {
			...
			Type type = this.formatTypeName(ctx);
			retorno += return_ + type.getTypePrefix() + "return\n"
					+ ".end method";
		}
	...
	public String visitMethodCall(MethodCallContext ctx) {
		...
		instructions += "invokestatic " + Main.ProgramName + "/"
				+ ctx.funcName1.getText() + "(";
		...
		while (num_par != 0) {
			...
			if (parametros_func.get(num_par - 1) != atual) {
				try {
					throw new Exception("Line: +" + ctx.start.getLine()
							+ "\n VOCE NAO PODE PASSAR UM PARAMETRO "
							+ atual.getType() + " SENDO QUE SE ESPERAVA UM "
							+ parametros_func.get(num_par - 1));
		...
		instructions += parametros.toString() + ")"
				+ methods.get(ctx.funcName1.getText()).getTypeParameter();
		return instructions;
	...
	private Type formatTypeName(MethodDefContext ctx) {
		if (ctx.typeVoid != null)
			return Type.valueOf(ctx.typeVoid.getText().substring(0, 1)
					.toUpperCase()
					+ ctx.typeVoid.getText().substring(1));
		return Type.valueOf(ctx.type_.getText().substring(0, 1).toUpperCase()
				+ ctx.type_.getText().substring(1));
	}
	...
	private void typeVerify_simpleAritm(int l) {
		...
		try {
			type_st.push(Type.Double);
			throw new Exception("line:" + l + " You can't operate with types "
					+ a + " and " + b + ", genius.");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void typeVerify_Div(int l) {
		...
		try {
			type_st.push(Type.Double);
			throw new Exception("line:" + l + "You can't operate with " + a
					+ " and " + b + ", genius.");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void typeVerify_equality(int l) {
		Type a = type_st.pop();
		Type b = type_st.pop();
		// if(a==Type.Double&&b==Type.Double || a==Type.Integer&&b==a ||
		// a==Type.Boolean&&a==b){
		if (a == Type.Double && b == a || a == Type.Int && b == a
				|| a == Type.Bool && a == b) {
			type_st.push(Type.Bool);
	...
	private void typeVerify_att(Type var, int l) {
		...
		try {
			type_st.push(var);
			throw new Exception("line: " + l + " \nYou can't attribute a " + a
					+ " type to variable of " + var + " type, genius.\n");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
